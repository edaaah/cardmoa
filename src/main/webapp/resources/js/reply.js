...
var replyService = (function() {
	...
	function add(reply, callback, error) {				// add 함수 선언 시작
		...
		$.ajax({											// ajax 시작
			type: 'post',
			url : '/replies/board/new',	// from 태그의 action과 같은 서버 주소
	...
	function getList(param, callback, error) {			// getList 함수 선언 시작
			...
			$.getJSON("/replies/board/pages/" + bno + "/" + page + ".json", 
				function(data) {
					if (callback) {
	...
	function get(rno, callback, error) {				// get 함수 선언 시작
		$.get("/replies/board" + rno + ".json", 	// get은 타입 (타입이 get인 것은 ajax를 줄여 쓸 수 있음 vs update의 ajax)
			function(result) {
				if (callback) {
					callback(result);
				}
